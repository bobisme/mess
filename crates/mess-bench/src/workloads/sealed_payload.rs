...
//! matches `phase5.sealed.bytes_per_event` /
//! `phase5.sealed.columnar_replay.ev_per_s` in `docs/perf/envelope.md`.
//!
...
pub fn run(size: RunSize) -> Vec<Metric> {
    ...
    vec![
        Metric::new(
            ...
            "B/event",
            format!(
                "columnar .pcol payload sidecar, D6 default (zstd-9, \
                 {DEFAULT_BLOCK_EVENTS}-event blocks, no dicts); \
                 {total}-event block-clustered corpus (~90% shreddable \
                 msgpack, ~10% binary row fallback); raw {:.1} B/event",
        ...
        Metric::new(
            ...
            replay_ev_per_s,
            "ev/s",
            "reassemble_all over the .pcol sidecar: columnar-decode all \
             events in stored order; single core, in-memory"
                .to_string(),
        ),
        Metric::new(
            ...
            seal_ev_per_s,
            "ev/s",
            "columnar seal encode + permanent verify-on-seal (every block \
             reassembled and byte-compared)"
                .to_string(),
        ),
    ]
}
