...
//!   spec-sanctioned partial state; a torn/partial trailer is no worse (a
//!   reader treats any `footer_crc` mismatch as unsealed anyway). Either way
//!   the handed [`SegmentSummary`] carries every fixed-trailer field
...
pub struct SegmentParams {
    ...
    pub epoch:              u64,
    /// Epoch of the immediately preceding segment, or `0` for the first
    /// (§3.2).
...
/// (`segment_id`/`base_pos`/`epoch`), `safe_offset` (→ `write_off`),
/// `next_pos`, `next_batch_id`, and the accepted-prefix counts.
#[derive(Debug, Clone, Copy)]
...
pub struct BatchSpec<'a, 'p> {
    ...
    /// `Some` sets `flags.CRYPTO_CHAIN` and writes these 32 bytes (§4.4).
    /// Phase 3 provides placement only; the chain **value** is Phase 5.
...
pub enum WriteError {
    ...
    #[error(
        "segment full: batch needs {needed} bytes, {remaining} remain before \
         segment_size"
    )]
    ...
    #[error(
        "store full: could not preallocate {requested} bytes for a new segment"
    )]
    StoreFull {
    ...
    #[error(
        "store poisoned: a prior fdatasync failed with ENOSPC; reopen and \
         recover"
    )]
    StorePoisoned,
...
fn is_enospc(e: &io::Error) -> bool { e.raw_os_error() == Some(libc::ENOSPC) }
...
pub struct SegmentWriter<F: Fs> {
    ...
    /// [`WriteError::StorePoisoned`] until the store is reopened and
    /// recovered.
    poisoned:      bool,
}
...
impl<F: Fs> SegmentWriter<F> {
    ...
    /// [`SEGMENT_SIZE`]).
    pub fn segment_size(&self) -> u64 { self.segment_size }
    ...
    /// live-auto-roll (`bn-1vu`) counterpart to
    /// [`close`](SegmentWriter::close), which consumes: the committer makes
    /// the just-full segment durable, then switches its live writer to the
    /// next segment ([`open_next`]) in place.
    ///
    ...
    /// (`base_pos = self.next_pos`), A9 chain (`prev_segment_epoch =
    /// self.epoch`, `next_epoch` strictly larger), and logical
    /// `segment_size` — **without consuming or closing** this writer. The
    /// caller MUST first make this segment durable
    /// ([`sync_and_summary`](SegmentWriter::sync_and_summary))
    /// and then replace its live writer with the returned one. Unlike
    /// [`roll`](SegmentWriter::roll)/
    /// [`roll_sealed`](SegmentWriter::roll_sealed) (which consume `self`
    /// and hardwire [`SEGMENT_SIZE`]), this preserves `self.segment_size`
    /// and leaves `self` intact so a failed preallocation
    ...
    /// before this returns leaves the segment trailer-less, i.e. unsealed,
    /// which recovery fully scans exactly as the active segment (§8.3); no
    /// committed batch is lost either way.
    ///
    ...
    /// Roll to the next segment, **sealing** the current one first (§3.1,
    /// §3.3): [`seal`](SegmentWriter::seal) this segment (write + fsync its
    /// footer), then [`create`](SegmentWriter::create) the next at
    /// `next_path` continuing the A1 chain (`base_pos = end_pos`) and the
    /// A9 chain (`prev_segment_epoch = this epoch`). `next_epoch` MUST be
    /// strictly larger than this segment's epoch (A9). This is the sealing
    /// counterpart of [`roll`](SegmentWriter::roll), which leaves the old
    /// segment unsealed.
    ///
    /// # Panics (debug)
    ///
    ...
    /// (`prev_segment_epoch = this epoch`). `next_epoch` MUST be strictly
    /// larger than this segment's epoch (A9).
    ///
...
pub fn read_segment_header_epoch(image: &[u8]) -> Option<u64> {
    ...
    let magic = u32::from_le_bytes(
        image[SH_MAGIC_OFF..SH_MAGIC_OFF + 4].try_into().ok()?,
    );
    let ver = u16::from_le_bytes(
        image[SH_FORMAT_VERSION_OFF..SH_FORMAT_VERSION_OFF + 2]
            .try_into()
            .ok()?,
    );
    ...
    let want = u32::from_le_bytes(
        image[SEGMENT_HEADER_CRC_OFF..SEGMENT_HEADER_CRC_OFF + 4]
            .try_into()
            .ok()?,
    );
    ...
    Some(u64::from_le_bytes(
        image[SH_EPOCH_OFF..SH_EPOCH_OFF + 8].try_into().ok()?,
    ))
}
...
mod resume_tests {
    ...
    fn resume_accepts_the_real_recovered_boundary() {
        ...
        SegmentWriter::resume(&fs, path, params)
            .expect("the real safe_offset must resume cleanly");
    }
    ...
    fn resume_rejects_write_off_past_file_len() {
        ...
        let err = match SegmentWriter::resume(&fs, path, params) {
            Ok(_) => panic!(
                "write_off past file len must be rejected in every build \
                 profile"
            ),
            Err(e) => e,
        };
        match err {
            ...
            other => {
                panic!("expected WriteError::InvalidResume, got {other:?}")
            }
        }
    }
    ...
    fn resume_rejects_write_off_before_header() {
        ...
        let err = match SegmentWriter::resume(&fs, path, params) {
            Ok(_) => panic!(
                "write_off before the header must be rejected in every build \
                 profile"
            ),
            Err(e) => e,
        };
