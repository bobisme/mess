...
//!
//! Every modeled attack MUST be detected. Where the spike promised frame-precise
//! errors (`ChainBreakFrameHash{i}`, `ChainBreakPrev{i+1}`) these are
//! **regenerated** for the batch-granular surface (§7.0, §11): the G10 layout
//! (§6.2) stores no per-frame hash, so a tail payload tamper localizes to the
//! *batch* — surfaced as `ChainBreakPrev{next batch base}` or, for the final
//! tail batch, `HeadMismatch`. The cross-stream attack (attack 6) MUST be caught
//! by the genesis `stream_id` binding (§3.1); the counterfactual (6b) proves the
//! binding is load-bearing by removing it and watching the same attack succeed.

use mess_log::certificates::*;
use mess_log::fold_chain::{advance, blob_hash, Hash};
...
fn snapshot_claiming_wrong_version_detected() {
    ...
    let err = load_verified::<Account>(&s, Some((&r, &blob))).unwrap_err();
    // Path A recomputes h[25] from frame 25 and it won't equal the stored h[19].
...
fn wrong_version_detected_by_path_b_when_frame_v_compacted() {
    // batch_size 13 => batches [0..12][13..25][26..38][39..49]. v=25 is the last
    // frame of batch 1; v+1=26 is the first frame of batch 2, so compacting
    // batch 1 removes frame 25 (Path A) but keeps frame 26 (Path B).
    let mut s = stream(3, 50, 13);
    let (mut r, blob) = take_snapshot::<Account>(&s, 19);
    r.stream_version = 25;
    s.batches.remove(1); // compact the batch holding frames 13..=25
    let err = load_verified::<Account>(&s, Some((&r, &blob))).unwrap_err();
    // Path A unavailable (frame 25 gone); Path B: frame 26's prev is h[25] != h[19].
    assert_eq!(err, VerifyError::PrefixHashMismatch { path: VerifyPath::FromFrameVPlus1 });
}
...
fn tampered_tail_event_with_no_stored_hash_still_caught_at_batch_boundary() {
    ...
    let (r, blob) = take_snapshot::<Account>(&s, 19);
    // There is nothing per-frame to "fix up" (§6.2); tamper is the whole attack.
...
// --- 6b. COUNTERFACTUAL: without stream_id in the genesis, the attack SUCCEEDS.
...
fn cross_stream_snapshot_accepted_without_genesis_binding() {
    ...
    // Build A's snapshot against the unbound genesis by hand (take_snapshot uses
    // the real bound genesis, which is exactly what this counterfactual drops).
    let mut h = unbound;
    ...
    for v in 0..=19u64 {
        let blk = a
            .batches
            .iter()
            .find(|bt| bt.base_version <= v && v < bt.base_version + bt.frames.len() as u64)
            .unwrap();
...
fn forged_prev_fooling_path_b_caught_by_tail_replay() {
    ...
    // (a) With frame 19 retained, Path A rejects first (recomputed h[19] != bogus).
    {
    ...
    // (b) With frame 19's batch compacted (Path A unavailable), Path B is fooled
    // by the forged crypto_chain — but the chain then breaks at the NEXT batch
    // boundary during tail replay (batch [30..39], base 30).
    {
        let mut s = stream(12, 50, 10);
        let (mut r, blob) = take_snapshot::<Account>(&s, 19);
        r.event_prefix_hash = bogus;
