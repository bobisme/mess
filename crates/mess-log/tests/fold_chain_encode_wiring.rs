...
use mess_log::fold_chain::{ChainHead, FrameChain, recompute_batch};
use mess_log::format::*;

/// `BatchHeader.flags` offset (spec 01 §4.2) — the crate-internal
/// `format::BH_FLAGS_OFF` is `pub(crate)`, so mirror it here for the byte
/// check.
...
fn real_chain_entry_encodes_at_offset_72_and_recomputes() {
    ...
    let flags = u16::from_le_bytes(
        bytes[BH_FLAGS_OFF..BH_FLAGS_OFF + 2].try_into().unwrap(),
    );
    ...
    assert_eq!(
        on_disk_chain, entry,
        "crypto_chain field holds the real h[base-1]"
    );
    ...
    let recomputed_exit =
        recompute_batch(&on_disk_chain, 0, refs.iter().copied(), |fc| {
            got.push(fc)
        });
...
fn chain_off_is_byte_identical_to_the_a4_only_path() {
    ...
    let flags = u16::from_le_bytes(
        off[BH_FLAGS_OFF..BH_FLAGS_OFF + 2].try_into().unwrap(),
    );
