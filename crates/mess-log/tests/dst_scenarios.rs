...
//! simulations composing the real
//! [`Committer`](mess_log::committer::Committer), subscriber-style readers, and
//! the sealer stand-in
...
//!   independent runs of the SAME seed produce byte-identical traces, including
//!   a CRC32C of every segment's final on-disk image.
//! - [`crash_lag_seal_race_composed`] — a DEDICATED scenario class that forces
//!   every fault/actor type named in the bone's acceptance criteria into ONE
//!   run: a `Sealed` segment (the sealer stand-in), laggy concurrent
//!   subscribers, and a fault landing on the seal barrier itself.

#[path = "dst_support/mod.rs"]
mod dst_support;

use dst_support::{SegKind, plan_scenario, run_scenario};

/// ~200 scenarios: cheap enough for every `cargo test -p mess-log`.
#[test]
#[cfg_attr(
    miri,
    ignore = "200 full scenarios (each a multi-segment sim run with \
              recover_segment + crc32c passes) are far too slow under Miri's \
              interpreter; same_seed_is_byte_identical below gives Miri a \
              real, cheap pass over the whole scenario path instead"
)]
fn dst_fast_profile() {
    ...
    println!(
        "dst fast profile: 200 scenarios, {committer_segs} Committer \
         segments, {sealed_segs} Sealed segments, {sealed_via_r2} trusted via \
         the R2 fast path"
    );
    assert!(
        committer_segs > 0,
        "fast profile never exercised a Committer-kind segment"
    );
    assert!(
        sealed_segs > 0,
        "fast profile never exercised a Sealed-kind segment"
    );
    assert!(
        sealed_via_r2 > 0,
        "fast profile never exercised the R2 (sealed) fast path"
    );
}
...
#[test]
#[ignore = "full 2k+-scenario profile: run via `cargo test -- --ignored` \
            (nightly CI)"]
fn dst_full_profile() {
    let mut committer_segs = 0u64;
    let mut sealed_segs = 0u64;
    ...
    println!(
        "dst full profile: {count} scenarios, {committer_segs} Committer \
         segments, {sealed_segs} Sealed segments, {sealed_via_r2} trusted via \
         the R2 fast path"
    );
}
...
fn same_seed_is_byte_identical() {
    ...
    let seeds: &[u64] =
        if cfg!(miri) { &[0, 42] } else { &[0, 1, 7, 42, 12345, 0xC0FFEE] };
    for &seed in seeds {
...
#[cfg_attr(
    miri,
    ignore = "150 full scenarios: too slow under Miri, same rationale as \
              dst_fast_profile"
)]
fn crash_lag_seal_race_composed() {
    ...
    for seed in 0..150u64 {
        ...
        assert!(
            recovered_next_pos >= acked_end,
            "seed {seed}: acked data lost across the seal-race crash"
        );
        assert!(
