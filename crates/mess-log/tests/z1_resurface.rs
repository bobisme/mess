...
//!      and in arbitrary order). Recovery scans, hits the `b0` hole, and
//!      stops dead (A10): `b1`'s bytes sit on disk but are NOT part of the
//!      committed prefix — dead space.
//!   3. The writer resumes at the recovered `safe_offset` and rewrites the
//!      hole with a same-shaped batch (a client's idempotent retry of the
//!      same logical append), and it is fsynced — genuinely acked this time.
...
//!    would (§2.3 `scan_batches`) — reflects exactly the two accepted
//!    batches' contribution: neither under- (the bait silently dropped) nor
//!    over-counted (the bait counted twice, or double-added on top of an
//!    unrelated batch).
//! 2. **Exactly-once delivery.** [`ReadView::read_committed`] (the real
//!    reader/subscription-history seam, `docs/spec/06-subscriptions.md` §3)
//!    over the final image serves each accepted position, and each
//!    `(stream_id, stream_version)` pair, exactly once.
//! 3. **The boundary condition.** [`z1_bait_stays_dead_when_rewrite_shape_differs`]
//!    is the negative control: mess-log itself has no time/capacity-bounded
//!    dedupe window (that lives in `mess-index`'s meta store, downstream of
//!    mess-log and out of this bone's scope — see the worker report). The
//!    "window" A6/Z1 actually turn on at this layer is purely the
//!    position/epoch realignment condition (Z1's own wording: "dead space is
//!    dead ... only until the positions line up again"). This test proves
//!    the resurfacing is conditional on exactly that: a rewrite of a
//!    DIFFERENT shape never re-aligns the bait's position, and the bait
//!    correctly stays dead forever.
...
use mess_log::scanner::{recover_segment, recover_segment_with_image, ScanStop};
...
fn build_hole_and_bait(
    ...
    assert!(
        ...
        "sanity: b0's header sector must be strictly before b1's persisted range"
    );
...
fn z1_double_crash_resurfaces_bait_and_is_absorbed_without_double_count() {
    ...
    assert!(rec1.accepted.is_empty(), "life-2 recovery must stop at the b0 hole");
    assert_eq!(rec1.safe_offset, SEGMENT_HEADER_LEN as u64);
    assert_eq!(rec1.next_pos, 0, "nothing committed yet — stream head must not move");
    assert!(rec1.stream_heads.is_empty(), "no stream head bump for a dead-on-arrival scan");
    assert_ne!(rec1.stop, ScanStop::EndOfSegment, "the hole is a real stop, not a clean tail");

    // ---- The writer resumes and rewrites the hole: a same-shaped retry of ----
    // the same logical append (same stream, same expected version, DIFFERENT
    // bytes — proving this is a genuinely new write, not b0's surviving
    // content). It is fsynced: truly acked this time.
    ...
    let r0b = w2
        .append(&BatchSpec {
            ...
            first_stream_version: 0,
            crypto_chain: None,
            subframes: &[Subframe::plain(0x11, 0, 0, &retry_payload)],
        })
        .unwrap();
    w2.sync().unwrap();

    assert_eq!(r0b.offset, r0.offset, "the rewrite lands exactly at the hole's old offset");
    assert_eq!(
    ...
    assert_eq!(rec2.stop, ScanStop::EndOfSegment, "clean tail: both slots now byte-valid");
    ...
    assert_eq!(bait.offset, r1.offset, "the bait is byte-identical to the original b1 write");
    ...
    assert_eq!(rec2.stream_heads[&STREAM], 1, "head == last_stream_version, events 0 and 1");
    let total_events: u64 = rec2.accepted.iter().map(|b| u64::from(b.frame_count)).sum();
    assert_eq!(total_events, 2, "exactly two events total — no loss, no duplication");
    ...
    for b in &rec2.accepted {
        for k in 0..u64::from(b.frame_count) {
            *version_counts.entry((b.stream_id, b.first_stream_version + k)).or_insert(0) += 1;
        }
    }
    assert_eq!(version_counts.len(), 2);
    assert!(version_counts.values().all(|&c| c == 1), "no (stream, version) counted twice");
    ...
    assert_eq!(committed_again, committed, "re-read of the same durable image is idempotent");
}
...
fn z1_bait_stays_dead_when_rewrite_shape_differs() {
    ...
    assert!(
        ...
        "bait must not resurface when the rewrite does not realign its position"
    );
    assert_eq!(rec2.accepted.len(), 1, "only the (2-event) rewrite is accepted");
    assert_eq!(rec2.accepted[0].frame_count, 2);
    assert_eq!(rec2.stream_heads.len(), 1);
    assert_eq!(
        rec2.stream_heads[&STREAM], 1,
        "head reflects the rewrite's own 2 events (0,1), not a 3rd resurfaced one"
    );
    let total_events: u64 = rec2.accepted.iter().map(|b| u64::from(b.frame_count)).sum();
    assert_eq!(total_events, 2, "the bait must not silently add a phantom 3rd event");
    let _ = r0;
}
