...
use mess_log::runtime::{
    CrashPlan, EnospcSite, Fault, FileHandle, Fs, OpenOpts, RealRuntime,
    Runtime, SectorPlan, SimFs,
};
...
fn allocate_enospc_at_create_yields_store_full_and_no_husk() {
    ...
    let err = match SegmentWriter::create(&fs, path, params(1, 0, 1)) {
        Ok(_) => panic!(
            "create must fail with StoreFull when preallocation hits ENOSPC"
        ),
        Err(e) => e,
    };
    assert!(
...
fn roll_enospc_yields_store_full_old_segment_recovers() {
    ...
    let err = match w.roll(seg1, 2, 2, 0) {
        Ok(_) => panic!(
            "roll must fail with StoreFull when preallocation hits ENOSPC"
        ),
        Err(e) => e,
    };
    assert!(
        matches!(err, WriteError::StoreFull { .. }),
        "roll must surface StoreFull, got {err:?}"
    );

    // No partial new segment: seg1 was created empty then removed.
    assert!(
        fs.open(seg1, OpenOpts::read_only()).is_err(),
        "no partial new segment"
    );

    // The old segment stayed durable and readable: recover its committed
    // prefix.
...
fn barrier_enospc_poisons_store_committed_prefix_recovers() {
    ...
    assert!(
        matches!(after, WriteError::StorePoisoned),
        "post-poison append: {after:?}"
    );
    let after_sync = w.sync().unwrap_err();
    assert_eq!(
        after_sync.kind(),
        std::io::ErrorKind::Other,
        "post-poison sync fails fast"
    );
    ...
    assert_eq!(
        rec.batch_count(),
        1,
        "only the pre-poison committed batch survives"
    );
    ...
    assert_eq!(
        rec.stop,
        ScanStop::BadMagic,
        "garbage tail rejected past the committed prefix"
    );
    let b0 = rec.accepted[0];
    assert_eq!(
        rec.safe_offset,
        b0.offset + b0.total_len,
        "resume at the committed prefix boundary"
    );
}
...
fn real_preallocation_keeps_size() {
    ...
    assert_eq!(
        f.len().unwrap(),
        sum.content_len,
        "KEEP_SIZE: st_size tracks content, not the reservation"
    );
    assert_eq!(
        sum.content_len,
        SEGMENT_HEADER_LEN as u64 + append_len(b"alpha")
    );
}
