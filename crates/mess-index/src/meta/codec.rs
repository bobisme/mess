...
//!   order equal byte order, so `stream_id` prefixes and the dedupe FIFO
//!   `seq` index iterate in the order recovery and eviction expect.
//! - **Values are little-endian, fixed-width, length-explicit.** They are
//!   never range-scanned, so endianness is a free choice; LE matches the
//!   project-wide default (D2). Every decoder validates length and rejects a
//!   short/garbled slice loudly rather than reading past the end — a
//!   corrupt fjall value is a bug or a torn write, never a silent zero.
...
pub enum DecodeError {
    ...
    #[error("metadata value too short: table {table}, need >= {need} bytes, got {got}")]
    TooShort {
...
#[must_use]
pub fn stream_key(id: StreamId) -> [u8; 8] {
    id.0.to_be_bytes()
}
...
pub fn decode_head(v: &[u8]) -> Result<Head, DecodeError> {
    if v.len() < 16 {
        return Err(DecodeError::TooShort { table: "stream_heads", need: 16, got: v.len() });
    }
    Ok(Head {
...
pub fn decode_snapshot(v: &[u8]) -> Result<SnapshotHead, DecodeError> {
    if v.len() < 16 {
        return Err(DecodeError::TooShort { table: "snapshot_heads", need: 16, got: v.len() });
    }
    Ok(SnapshotHead {
...
#[must_use]
pub fn encode_pos(pos: u64) -> [u8; 8] {
    pos.to_le_bytes()
}
...
pub fn decode_dedupe(v: &[u8]) -> Result<(u64, u64), DecodeError> {
    if v.len() < 16 {
        return Err(DecodeError::TooShort { table: "dedupe", need: 16, got: v.len() });
    }
    Ok((
