...
async fn inspect_reports_offline_metrics() {
    ...
    let json: Value = serde_json::from_str(&format::render(&report, Format::Json)).unwrap();
    ...
    assert!(m["total_size_bytes"].as_u64().unwrap() > 0, "segment has bytes on disk");
    ...
    assert!(
        ...
        "inspect must document that runtime metrics are in-process only: {advice:?}"
    );
}
