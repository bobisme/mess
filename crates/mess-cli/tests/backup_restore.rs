...
fn rec(data: &[u8]) -> RecordToAppend {
    RecordToAppend { message_type: "account.happened".into(), data: data.to_vec() }
}

fn small_engine(dir: &Path) -> LogEngine {
    LogEngine::open_with(dir, EngineOptions { segment_size: 1 << 20, ..EngineOptions::default() })
        .expect("open engine")
}
...
fn backup_during_active_writing_restores_to_the_cut() {
    ...
    let backup_json = rt.block_on(async {
        ...
        for i in 0..10u64 {
            ...
            engine
                .append_batch("acct-1", expected, &[rec(format!("e{i}").as_bytes())])
                .await
                .expect("append");
        }
        // The writer is LIVE (engine open, lock held) while the backup runs.
        let report = backup::run(src.path(), dest.path(), &BackupOptions::default());
        assert_eq!(report.exit_code(), 0, "backup should succeed: {:?}", report.findings);
        let json = report.to_json();
        // The writer keeps appending AFTER the cut — these must not appear in
        // the backup.
        for i in 10..15u64 {
            engine
                .append_batch("acct-1", Version::At(i - 1), &[rec(format!("e{i}").as_bytes())])
                .await
    ...
    let watermark = field(&backup_json, "watermark").as_u64().expect("watermark");
    assert_eq!(watermark, 10, "cut watermark is the 10 events acked before the cut");
    ...
    let rr = restore::run(dest.path(), restored.path(), &RestoreOptions::default());
    assert_eq!(rr.exit_code(), 0, "restore should be clean: {:?}", rr.findings);
    let rj = rr.to_json();
    assert_eq!(field(&rj, "verified"), &Value::Bool(true), "verify --full clean");
    ...
    runtime().block_on(async {
        ...
        assert_eq!(engine.total_events(), 10, "restored store holds only the pre-cut events");
...
fn torn_tail_from_a_crashed_writer_is_excluded_from_the_cut() {
    ...
    let scan = mess_cli::scan::scan_segment(common::SEG_ID, &log).expect("scan");
    let safe = scan.recovery.safe_offset;
    // Overwrite bytes just past the watermark with a torn/garbage batch header.
    {
        use std::io::{Seek, SeekFrom, Write};
        let mut f = std::fs::OpenOptions::new().write(true).open(&log).expect("open log");
    ...
    let report = backup::run(src.path(), dest.path(), &BackupOptions::default());
    assert_eq!(report.exit_code(), 0, "backup ignores the torn tail: {:?}", report.findings);
    let watermark = report.to_json()["watermark"].as_u64().expect("watermark");
    assert_eq!(watermark, 8, "cut watermark is the 8 committed events, tail excluded");

    let rr = restore::run(dest.path(), restored.path(), &RestoreOptions::default());
    assert_eq!(rr.exit_code(), 0, "restore clean despite the crash artifact: {:?}", rr.findings);
...
fn incremental_copies_only_new_segments() {
    ...
    let copied_segment_files = |r: &Report| -> usize {
        r.collection
            ...
            .filter(|row| {
                ...
                (role == "sealed" || role == "sidecar") && row["action"] == "copied"
            })
            .count()
    };

    // Full backup copies seg-1 and its sidecars.
    let full = backup::run(src.path(), dest.path(), &BackupOptions { incremental: false });
    ...
    let noop = backup::run(src.path(), dest.path(), &BackupOptions { incremental: true });
    assert_eq!(noop.exit_code(), 0);
    assert_eq!(copied_segment_files(&noop), 0, "no segment re-copied when nothing changed");
    assert!(noop.to_json()["skipped_files"].as_u64().unwrap() >= 1, "prior segment files skipped");
    ...
    let incr = backup::run(src.path(), dest.path(), &BackupOptions { incremental: true });
    ...
    for row in &incr.collection {
        ...
        if (role == "sealed" || role == "sidecar") && row["action"] == "copied" {
            ...
            assert!(path.contains("0002"), "only seg-2 segment files copied: {path}");
        }
    }
}
...
fn missing_manifest_makes_restore_refuse() {
    ...
    std::fs::remove_file(dest.path().join(BACKUP_MANIFEST)).expect("remove manifest");

    let rr = restore::run(dest.path(), restored.path(), &RestoreOptions::default());
    assert_ne!(rr.exit_code(), 0, "restore must refuse a torn backup");
    assert!(has_kind(&rr, "torn-backup"), "surfaces torn-backup: {:?}", rr.findings);
}
...
fn restore_refuses_non_empty_target() {
    ...
    std::fs::write(target.path().join("existing"), b"do not clobber").expect("seed target");

    let rr = restore::run(dest.path(), target.path(), &RestoreOptions::default());
...
fn retention_lease_blocks_deletion_during_backup_and_releases_after() {
    ...
    let seg_row = |r: &Report| -> Value {
        r.collection.iter().find(|row| row["segment_id"] == common::SEG_ID).cloned().expect("seg row")
    };
    assert_eq!(seg_row(&before)["verdict"], "deletable");

    // Hold a lease covering seg-1 (as `mess backup` does for its cut).
    let guard = lease::acquire(src.path(), "b-test", 1, 1, 5, lease::DEFAULT_TTL_SECS)
        .expect("acquire lease");
    let during = retention::run(src.path());
    assert_eq!(seg_row(&during)["verdict"], "blocked", "lease pins the segment");
    assert!(has_kind(&during, "lease-hold"), "lease-hold surfaced: {:?}", during.findings);
    ...
    assert_eq!(seg_row(&after)["verdict"], "deletable", "released lease unblocks");
...
fn add_sealed_segment_copy(dir: &Path, from_id: u64, to_id: u64) {
    ...
    for ext in ["pidx", "pcol", "filter"] {
        ...
        if f.exists() {
            std::fs::copy(&f, to_pidx.with_extension(ext)).expect("copy sidecar");
        }
    }
}
