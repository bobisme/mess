...
fn rebuild_index_reproduces_pidx_byte_for_byte() {
    ...
    let original = std::fs::read(&pidx).expect("original .pidx exists after seal");
    ...
    let report = rebuild::run(d.path(), &RebuildOptions { dry_run: false, meta: false });
    assert_eq!(report.exit_code(), 0, "rebuild must succeed: {:#?}", report.findings);

    let rebuilt = std::fs::read(&pidx).expect(".pidx rebuilt");
    assert_eq!(rebuilt, original, "rebuilt .pidx must be byte-equal to the sealer's output");
}
...
fn rebuild_is_deterministic() {
    ...
    let r1 = rebuild::run(d.path(), &RebuildOptions { dry_run: false, meta: false });
    assert_eq!(r1.exit_code(), 0);
    let first = std::fs::read(&pidx).expect("rebuilt once");

    let _ = std::fs::remove_file(&pidx);
    let r2 = rebuild::run(d.path(), &RebuildOptions { dry_run: false, meta: false });
