...
//!   named streams, a tiny (16 KiB) active segment so the log genuinely
//!   rolls and the background sealer produces sealed segments (columnar
//!   `.pcol` + pointer `.pidx` + a finalized fixed trailer), the fjall meta
//!   name registry (`stream_names`/`type_names`), and a snapshot carrying a
//!   `fold_version` in a [`FjallSnapshotBackend`] nested under the store dir.
//!   It records an `expected-events.json` manifest and packs the store dir as
//!   `store.tar.zst` under `tests/golden/v3/`.
//!
//! - [`golden_v3_opens_and_verifies`] (runs in normal CI) unpacks the
//!   committed v3 golden, opens it with the CURRENT code, and proves: full
//!   recovery scan is green, the registry + names hydrated, every event
//!   replays byte-exact against the manifest, sealed streams are served from
//!   the COLD tier, the snapshot loads with its `fold_version`, `load_verified`
//!   is green on the chained stream (with its fold-chain hashes pinned in the
...
/// The interned stream id fed to the fold-chain genesis (§3.1). Fixed + recorded.
...
/// Deterministic payload for stream index `s`, stream version `v`. No wall-clock;
/// varies in length and content so a byte-exact replay is meaningful.
fn payload(s: usize, v: usize) -> Vec<u8> {
    let mut out = format!("ord{s:02}-v{v:04}-").into_bytes();
    let len = 8 + (v % 12);
    out.extend((0..len).map(|k| b'a' + u8::try_from((s + v + k) % 26).unwrap()));
    out
}
...
impl Aggregate for SumAgg {
    ...
    fn init() -> Self {
        SumAgg { sum: 0, count: 0 }
    }
    fn apply(&mut self, payload: &[u8]) {
        self.sum = self.sum.wrapping_add(payload.iter().map(|&b| u64::from(b)).sum::<u64>());
        self.count += 1;
    }
...
fn sh(cmd: &str) {
    let status = Command::new("sh").arg("-c").arg(cmd).status().expect("spawn sh");
...
// ===========================================================================
// GENERATOR (run explicitly:  cargo test -p mess-cli --test golden -- --ignored --nocapture generate_golden_v3)
// ===========================================================================
...
async fn generate_golden_v3() {
    ...
    // ---- 1. Build the store with the production engine (rolling segments). ----
    {
            ...
            if s == CHAINED_STREAM_IDX {
                ...
                while v < EVENTS_PER_STREAM {
                    ...
                    let recs: Vec<RecordToAppend> = (0..n)
                        ...
                        .collect();
                    let out = engine.append_batch(&name, expected, &recs).await.expect("append");
                ...
                for v in 0..EVENTS_PER_STREAM {
                    let out = engine
                        .append_batch(&name, expected, &[rec(message_type(v), &payload(s, v))])
                        .await
                        .expect("append");
                    expected = out.version;
                }
        ...
        let global = engine.read_global(None, STREAMS * EVENTS_PER_STREAM * 2).await.expect("read");
        assert_eq!(global.len(), STREAMS * EVENTS_PER_STREAM, "every event committed");
        ...
        let cert = build_cert(CHAINED_STREAM_ID, &chained_payloads, CHAINED_BATCH_SIZE, None);
        let (head_ver, head_hash) = cert.head_anchor.expect("head anchor");
        assert_eq!(head_ver, EVENTS_PER_STREAM as u64 - 1);
        let (snap_ref, snap_blob) = take_snapshot::<SumAgg>(&cert, SNAPSHOT_VERSION);
        // Sanity: the honest snapshot verifies against the honest cert now.
        let out = load_verified::<SumAgg>(&cert, Some((&snap_ref, &snap_blob))).expect("verify");
        assert!(!out.rebuilt_by_replay);
        let expected_state = SumAgg::from_bytes(&out.state.to_bytes()).expect("state");

        // ---- 3. Save a snapshot (with fold_version) into the nested store. ----
        {
            let snaps = FjallSnapshotBackend::open(engine.clone(), store.join("snapshots"))
            ...
            snaps.save_snapshot(CHAINED_STREAM, stored).await.expect("save snapshot");
    ...
    let sealed_pidx = std::fs::read_dir(store.join("sealed"))
        .expect("sealed dir")
        .flatten()
        .filter(|e| e.path().extension().and_then(|x| x.to_str()) == Some("pidx"))
        .count();
...
// ===========================================================================
// CHECK (normal CI:  cargo test -p mess-cli --test golden golden_v3_opens_and_verifies)
// ===========================================================================
...
async fn golden_v3_opens_and_verifies() {
    ...
    let manifest: Value =
        serde_json::from_slice(&std::fs::read(&manifest_path).expect("read manifest")).expect("parse");
    ...
    for sh in manifest["stream_heads"].as_array().expect("stream_heads") {
        ...
        assert_eq!(engine.head(name).await.unwrap(), Version::At(head), "head for {name}");
    }
    ...
    for (got, want) in global.iter().zip(events) {
        assert_eq!(got.global_position, want["g"].as_u64().unwrap(), "global_position");
        assert_eq!(got.stream_id, want["stream"].as_str().unwrap(), "stream name (registry)");
        assert_eq!(got.message_type, want["type"].as_str().unwrap(), "message type (registry)");
        assert_eq!(got.stream_position, want["sv"].as_u64().unwrap(), "stream_version");
        assert_eq!(got.data, from_hex(want["data"].as_str().unwrap()), "payload bytes");
    }

    // ---- 5. Cold reads: a fully-sealed stream is served byte-exact. ----
    let cold_name = manifest["cold_stream"].as_str().unwrap();
    let cold = engine.read_stream(cold_name, Version::NoStream, total).await.unwrap();
    let cold_expected: Vec<&Value> =
        events.iter().filter(|e| e["stream"].as_str().unwrap() == cold_name).collect();
    ...
    for (got, want) in cold.iter().zip(&cold_expected) {
        ...
        assert_eq!(got.data, from_hex(want["data"].as_str().unwrap()), "cold payload byte-exact");
    }
    ...
    let mut chained_payloads: Vec<(u64, Vec<u8>)> = events
        ...
        .map(|e| (e["sv"].as_u64().unwrap(), from_hex(e["data"].as_str().unwrap())))
        .collect();
    chained_payloads.sort_by_key(|(v, _)| *v);
    let payloads: Vec<Vec<u8>> = chained_payloads.into_iter().map(|(_, p)| p).collect();
    ...
    assert!(!out.rebuilt_by_replay, "honest snapshot should not force a rebuild");
    assert_eq!(
        out.state.sum,
