...
impl Block {
    ...
    /// Returns next offset
    pub fn put(&mut self, op: Op) -> Result<usize> {
...
mod test {
    ...
    fn build_block() -> Block {
        ...
        for _ in 0..20 {
            block.put(Op::Put(b"hello there"[..].into())).unwrap();
            block.put(Op::Put(b"what's up?"[..].into())).unwrap();
        }
        block
    }
    ...
    fn put_errors_when_write_would_overflow_block() {
        ...
        for _ in 0..117 {
            block
                .put(Op::Put(b"let's overflow this sucker"[..].into()))
                .unwrap();
        }
        let res =
            block.put(Op::Put(b"this one should actually overflow"[..].into()));
