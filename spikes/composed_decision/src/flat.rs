//! **FlatEngine** — a single-owner flat-combined append/publish prototype
//! over the REAL `mess-log` committer.
//!
//! PROVENANCE: this file is `spikes/flat_combined_append/src/lib.rs` (Spike B,
//! bn-28g) carried forward VERBATIM except for one API-drift fix, so that
//! Spike J (bn-2gu) can re-run Spike B's matched matrix against TODAY's main
//! engine. Spike B measured against workspace base `a3d40ff1`, i.e. BEFORE
//! bn-34o (name-persist coalescing), bn-2cj (durability-gated name barrier)
//! and bn-2ib/Spike C (Book removal) landed; its "flat beats the engine"
//! numbers are therefore stale and must be re-taken. The frozen spike is left
//! untouched (`spikes/README.md`: spikes are reference artifacts).
//!
//! The ONE change vs the frozen original:
//!   * `AppendOutcome::Acked` gained `segment_id` / `offset` fields on main,
//!     so the owner's ack match arm now ends in `..`. No behaviour change.
//!
...
impl FlatEngine {
    pub fn open(
        ...
        let writer =
            SegmentWriter::create(&fs, &path, params).map_err(|e| {
                std::io::Error::other(format!("segment create: {e}"))
            })?;
...
impl Owner {
    ...
    fn gather(
        ...
        let cap = match self.cfg.durability {
            ...
            Durability::Os | Durability::Process => (self.cfg.spin_gather_us
                > 0)
...
enum Core {
    Committer { committer: Committer<RealRuntime>, appender: Appender },
    Direct { writer: SegmentWriter<RealFs> },
}
...
fn run_b0_direct(
    ...
    while let Some(first) = owner.recv_intake(rx) {
        ...
        type Done = oneshot::Sender<Result<Appended, AppendError<FlatError>>>;
