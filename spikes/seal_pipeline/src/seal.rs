//! The seal pass (D5/D6) and the sealed-segment reader.
//!
//! Sealed layout choices made by this spike:
//!
//! - Payloads are rewritten CLUSTERED BY (category, stream, version): each
//!   ~128-event block is category-homogeneous (so per-category dictionaries
//!   apply cleanly, D6) and a stream's events are contiguous within its
//!   category section — the same locality an LSM buys with stream-prefixed
//!   keys and compaction. Global-position order is what the retained log
//!   segment is for; the sealed artifacts serve stream reads.
//! - Every event gets a "sealed slot": eslot = block_id * 128 + index_in_block
//!   (blocks are only short at category boundaries, so idx < 128 always).
//! - Per-stream packed pointer block = varint(first eslot) + varint deltas.
//!   With clustering the deltas are all 1 (~1 byte/event); the encoding stays
//!   general for layouts that don't cluster.
//!
//! Sealed file layout:
//!
//! ```text
...
//! footer:      9 x u64: ptrs_off, dir_off, bidx_off, dicts_off, filters_off,
//!              n_events, n_streams, block_events, magic
//! ```
...
pub const FILE_MAGIC: u64 = 0x5EA1_ED00_0000_0001;
const FOOTER_LEN: u64 = 9 * 8;
...
/// The per-event active index: (stream, version) -> EvRef. This is the seal
/// pass INPUT (an in-memory stand-in for the fjall per-event keyspace; the
/// index backend is not the subject here).
...
pub fn train_dicts(seg: &[u8], streams: &[StreamEvents]) -> DictOut {
    ...
    let mut samples_n = Vec::with_capacity(CATEGORIES);
    let mut sample_bytes = Vec::with_capacity(CATEGORIES);
    for cat in 0..CATEGORIES {
        ...
        let bytes: u64 = samples.iter().map(|s| s.len() as u64).sum();
        let t = Instant::now();
        let dict = zstd::dict::from_samples(&samples, DICT_MAX_BYTES).expect("dict training");
        train_times.push(t.elapsed());
        samples_n.push(samples.len());
        sample_bytes.push(bytes);
        dicts.push(dict);
    }
    DictOut { dicts, train_times, samples: samples_n, sample_bytes }
}
...
/// streaming them into `w` (which is positioned at `file_off0`).
pub fn recompress(
    seg: &[u8],
...
pub fn write_metadata(
    ...
    let mut dir_entries: Vec<(u64, u32, u32, u32)> = Vec::with_capacity(streams.len());
    for (si, se) in streams.iter().enumerate() {
    ...
    // Stream directory.
    let dir_off = ptrs_off + sections.ptrs;
    let mut buf: Vec<u8> = Vec::with_capacity(4 + dir_entries.len() * 20);
    buf.extend((dir_entries.len() as u32).to_le_bytes());
    for &(s, count, off, len) in &dir_entries {
    ...
    let mut buf: Vec<u8> = Vec::with_capacity(4 + blocks.len() * 27);
...
impl SealedReader {
    pub fn open(path: &Path) -> Self {
        ...
        let dir_off = rd_u64(&foot, 8);
        let bidx_off = rd_u64(&foot, 16);
        let dicts_off = rd_u64(&foot, 24);
        let filters_off = rd_u64(&foot, 32);
        let n_events = rd_u64(&foot, 40);
        assert_eq!(rd_u64(&foot, 56), BLOCK_EVENTS as u64);
        assert_eq!(rd_u64(&foot, 64), FILE_MAGIC);
        ...
        let ptr_data = meta[..at(dir_off)].to_vec();
        ...
        for _ in 0..n_streams {
            ...
            p += 20;
            dir.insert(s, DirEntry { count, off, len });
            stream_ids.push(s);
        }
...
pub struct ReadCtx<'d> {
    decs: Vec<zstd::bulk::Decompressor<'d>>,
...
/// eslot, then one block fetch (cache-aware).
