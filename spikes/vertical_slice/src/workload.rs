//! Seeded workload: identical logical event sequence for both engines.
//! ~250-byte JSON-ish payloads, 10,000 streams Zipf(s=1.1), batches of 10,
//! each stream owned by one of 4 writers (stream % WRITERS).
...
pub fn generate(n_events: usize, seed: u64) -> (Vec<BatchSpec>, Vec<u64>) {
...
/// Per-stream event counts induced by the first `n_batches` of `batches`.
pub fn counts_for_prefix(batches: &[BatchSpec], n_batches: usize) -> Vec<u64> {
    let mut counts = vec![0u64; STREAMS as usize];
    for b in &batches[..n_batches] {
        counts[b.stream as usize] += b.payloads.len() as u64;
    }
    counts
}

/// Partition the first `n_batches` among writers by stream ownership,
/// preserving per-stream order. Returns index lists into `batches`.
pub fn partition(batches: &[BatchSpec], n_batches: usize, writers: usize) -> Vec<Vec<u32>> {
    let mut parts = vec![Vec::new(); writers];
    for (i, b) in batches[..n_batches].iter().enumerate() {
