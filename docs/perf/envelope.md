...
| phase5.foldchain.append_overhead.gate_ns | 510 | quiet-host admission ceiling (3.0× the 169.3 ns/event reference); median delta of 5 paired 1M-event samples; the normal suite uses a deterministic construction test instead of wall time | 2026-07-14 | `cargo test -p mess-log --release --test fold_chain_overhead chain_append_overhead_is_in_envelope -- --ignored --nocapture` |
